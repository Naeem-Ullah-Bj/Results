<script>
// WRAP ENTIRE SCRIPT IN IIFE FOR BLOGGER COMPATIBILITY
(function() {
    // 彩票应用的主要脚本
    ...
    function initLotteryApp() {
        console.log('Lottery app initializing on Blogger...');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initLotteryApp);
    } else {
        // DOM already ready
        setTimeout(initLotteryApp, 100);
    }

    function lotteryUpdateLastUpdated() {
        const updateElement = document.getElementById('lotteryUpdateTime');
        if (!updateElement) {
            console.log('Update element not found yet');
            return;
        }
        ...
        if (!success) {
            ...
            if (lastData) {
                try {
                    ...
                    const warningContainer = document.getElementById('lotteryWarningContainer');
                    if (warningContainer) {
                        warningContainer.innerHTML = `<div class="lottery-custom-warning" style="background:#fffbeb; border:1px solid #fbbf24; color:#92400e; padding:12px; border-radius:8px; margin:15px 0;">${warningMsg}</div>`;
                    }
    ...
    function lotteryClearAllData() {
        ...
        const updateElement = document.getElementById('lotteryUpdateTime');
        if (updateElement) updateElement.textContent = 'Never';
        
        const dataSourceElement = document.getElementById('lotteryDataSource');
        if (dataSourceElement) dataSourceElement.textContent = '';
    ...
    // Expose functions to global scope for Blogger compatibility
    ...
    window.lotteryProcessData = lotteryProcessData;
    window.lotteryUpdateLastUpdated = lotteryUpdateLastUpdated;
